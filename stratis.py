...
miners=[]
stats=[]
f_w=open("stratis.txt",'w')

for j in range(10000):
	page='https://chainz.cryptoid.info/strat/block.dws?'+str(1033269-j)+'.htm'
	page=requests.get(page)
	html = page.text
	i=html.find("blockHash")
	blockhash=html[i+13:i+77]


	page='https://chainz.cryptoid.info/explorer/block.txs.dws?coin=strat&h='+blockhash+'.js'
	page=requests.get(page)
	html = page.text
	i=html.find('"inputs":[{"a":')

	miner=html[i+16:i+50]
	print(j)
