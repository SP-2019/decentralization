...
miners=[]
stats=[]
f_w=open("komodo.txt",'w')

for j in range(10000):

	page='https://explorer.komodo.services/api/block-index/'+str(1054000-j)
	...
	page='https://explorer.komodo.services/api/block/'+bxhash
	page=requests.get(page)
	html = page.text
	
	i=html.find('"poolName":"')
	if i!=-1:
		html=html[i+len('"poolName":"'):]
		i=html.find('"')
		miner=html[:i]
	else:
		page='https://explorer.komodo.services/api/txs?block='+bxhash
		page=requests.get(page)
		html = page.text
		...
		while value<1.5:
			i=html.find('"value":"')
			html=html[i+len('"value":"'):]
			i=html.find('"')
			value=float(html[0:i])
		i=html.find('"addresses":["')
		html=html[i+len('"addresses":["'):]
		i=html.find('"')
		miner=html[:i]
