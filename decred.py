...
miners=[]
stats=[]
f_w=open("decred.txt",'w')

for j in range(10000):
	page='https://explorer.dcrdata.org/block/'+str(283028-j)
	page=requests.get(page)
	html = page.text
	i = html.find('<a class="hash" href="/tx/')
	html=html[i+len('<a class="hash" href="'):]
	i = html.find('"')
	txhash=html[:i]
	
	page='https://explorer.dcrdata.org'+txhash
	page=requests.get(page)
	html = page.text
	
	value=0
	while value<10: 
		i = html.find('<a href="/address/')
		html=html[i+len('<a href="/address/'):]
		i = html.find('"')
		miner=html[:i]
		i=html.find('<span class="int">')
		html=html[i+len('<span class="int">'):]
		i=html.find('<')
		value=int(html[:i])
