...
for j in range(100):
	...
	page='https://etherscan.io/blocks?ps=100&p='+str(j+1)
	page=requests.get(page)
	html = page.text
	height=0
	if j==0:
		i=html.find('/block/')
		html=html[i+len('/block/'):]
		i=html.find("'")
		height=int(html[:i])
	else:	
	...
	height2=height
	i=0
	while html.find('address-tag')!=-1:
		i=html.find('address-tag')
		html2=html[:i]
		html=html[i+len('address-tag'):]
		i=html2.find('/address/')
		html2=html2[i+len('/address/'):]
		i=html2.find("'")
		miner=html2[:i]
