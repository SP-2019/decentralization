...
miners=[]
stats=[]
f_w=open("electra.txt",'w')

for j in range(10):
	bxhash=[]
	page='https://cryptobe.com/chain/Electra?hi='+str(117434-j*1000)+'&count=1000'
	...
	while html.find('a href="')!=-1:
		...
		html=html[i+10:]
		i=html.find('"')
		bxhash.append(html[:i])
	
	for k in range(len(bxhash)):
		page='https://cryptobe.com'+bxhash[k]
		page=requests.get(page)
		html = page.text
		i=html.find('To (amount)')
		html=html[i:]
		i=html.find('</table>')
		html=html[:i]
		value=0
