...
miners=[]
stats=[]
f_w=open("elastos.txt",'w')

for j in range(10000):

	page='https://blockchain.elastos.org/api/v1/block-index/'+str(223531-j)
	...
	bxhash=html[15:i]
	page='https://blockchain.elastos.org/api/v1/txs/?block='+bxhash+'&pageNum=0'
	page=requests.get(page)
	html = page.text
	check='false'

	while check=='false':
		i=html.find('"isCoinBase"')
		html=html[i+14:-1]
		i=html.find(',')
		check=html[0:i]
	...
	while i!=-1:
		...
		html=html[i+14:]
		i=html.find('"')
		if html[0:i] not in miner2:
			miner2.append(html[0:i])
		i=html.find('addresses')
			
	for i in range(len(miner2)):
		if i==0:
			miner=miner2[0]
		else:
			miner=miner+'&'+miner2[i]
