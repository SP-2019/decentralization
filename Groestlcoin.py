...
miners=[]
stats=[]
f_w=open("groestl.txt",'w')

for j in range(10000):
	
	page='https://chainz.cryptoid.info/grs/'+str(2295766-j)+'.htm'
	...
	if k==-1:	
		page='https://chainz.cryptoid.info/grs/api.dws?q=getblockhash&height='+str(2295766-j)
		page=requests.get(page)
		html = page.text
		while html.find('wait 5')!=-1:
			sleep(360)
			...
			html = page.text
		bxhash=html[1:-1]
		page='https://chainz.cryptoid.info/explorer/block.txs.dws?coin=grs&h='+bxhash+'.js'
		page=requests.get(page)
		html = page.text
		while html.find('wait 5')!=-1:
			sleep(360)
