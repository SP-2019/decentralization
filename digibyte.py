...
miners=[]
stats=[]
f_w=open("digibyte.txt",'w')

for j in range(10000):
	
	page='https://chainz.cryptoid.info/dgb/block.dws?'+str(7486827-j)+'.htm'
	...
	if k==-1:	
		page='https://chainz.cryptoid.info/dgb/api.dws?q=getblockhash&height='+str(7486827-j)
		page=requests.get(page)
		html = page.text
		while html.find('wait 5')!=-1:
			sleep(360)
			...
			html = page.text
		bxhash=html[1:-1]
		page='https://chainz.cryptoid.info/explorer/block.txs.dws?coin=dgb&h='+bxhash+'.js'
		page=requests.get(page)
		html = page.text
		i = html.find('"a":')
		html=html[i+5:-1]
