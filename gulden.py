...
miners=[]
stats=[]
f_w=open("gulden.txt",'w')

for j in range(10000):

	page='http://guldenchain.com/api/block-index/'+str(827094-j)
	...
	bxhash=html[14:i]
	page='http://guldenchain.com/api/block/'+bxhash
	page=requests.get(page)
	html = page.text
	...
	if i!=-1:
		...
		page='http://guldenchain.com/api/txs?block='+bxhash
		page=requests.get(page)
		html = page.text

		i=html.find('cbAddresses')
		html=html[i+14:-1]
		i=html.find('"')
		miner=html[0:i]
