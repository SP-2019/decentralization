...
miners=[]
stats=[]
f_w=open("bitcoin.txt",'w')
for j in range(2000):

	page='https://www.blockchain.com/en/btc/block-height/'+str(546057-j)
	page=requests.get(page)
	html = page.text
	
	i=html.find('Relayed By</td>')
	if i!=-1:
		html=html[i+len('Relayed By</td>'):]
		i=html.find('</')
		html=html[:i]
                if html.find('">')!=-1:
                    i=html.find('">')
		...
		page='https://blockchain.info/block-height/'+str(546057-j)+'?format=json'
		page=requests.get(page)
		html = page.text	
		i=html.find('"addr":"')
		html=html[i+len('"addr":"'):]
		i=html.find('"')
		miner=html[:i]
