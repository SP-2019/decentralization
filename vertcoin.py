...
for j in range(5121,10000):
	
	page='https://chainz.cryptoid.info/vtc/block.dws?'+str(1016656-j)+'.htm'
	...
	k=html.find("extracted by")
	
	if k==-1:	
		page='https://chainz.cryptoid.info/vtc/api.dws?q=getblockhash&height='+str(1016656-j)
		page=requests.get(page)
		html = page.text
		while html.find('wait 5')!=-1:
			sleep(360)
			...
			html = page.text
		bxhash=html[1:-1]
		page='https://chainz.cryptoid.info/explorer/block.txs.dws?coin=vtc&h='+bxhash+'.js'
		page=requests.get(page)
		html = page.text
		while html.find('wait 5')!=-1:
			sleep(360)
