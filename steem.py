...
miners=[]
stats=[]
f_w=open("steem.txt",'w')


for j in range(100):
	page='https://api.steemit.com/'
	height=26819046-j
	data_request={'id':4,"jsonrpc":"2.0","method":"call","params":["database_api","get_block",[height]]}	
	page=requests.post(page, data=json.dumps(data_request))
	html = page.text
	

	i = html.find('"witness":')
	i2=html.find('","transaction_merkle_root"')
	miner=html[i+11:i2]
	print(j)
